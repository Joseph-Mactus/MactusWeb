                ...
                <div className="absolute top-full left-0 mt-2 w-56 bg-white/90 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-xl overflow-hidden opacity-0 invisible group-hover/nav:opacity-100 group-hover/nav:visible group-hover/nav:translate-y-0 translate-y-4 transition-all duration-500 ease-out z-[60] border border-white/40">
                  ...
                  <div className="py-3 px-2">
                    {item.subItems.map((sub, idx) => {
                      let path = "#";
                      if (sub === "Smart Access Control System") path = "/products/sacs";
                      if (sub === "Intervention Recording System") path = "/products/irs";
                      if (sub === "Automated Solution Dispensing System") path = "/products/asds";
                      if (sub === "MEM™ (Environmental Monitoring)") path = "/products/mem";
                      ...
                      if (sub === "Case Studies") path = "/case-studies";
                      if (sub === "Building Management System") path = "/system-integration/building-management-system";
                      if (sub === "Environmental Monitoring System") path = "/system-integration/environmental-monitoring-system";
                      if(sub==="Testimonials") path = "/testimonial";
                      if(sub==="Contact Us") path = "/contact-us";
                      if(sub==="Low Voltage Systems") path = "/system-integration/low-voltage-systems";
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <div key={item.title} className="border-b border-gray-100 last:border-0 pb-4">
                    {topPath ? (
                      <a href={topPath} onClick={() => setIsMenuOpen(false)}>{mobileContent}</a>
                    ) : (
                      mobileContent
                    )}
