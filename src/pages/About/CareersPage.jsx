...
const CareersPage = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        const query = `*[_type == "jobPosting" && active == true] | order(order asc, _createdAt desc) {
        _id,
        title,
        location,
        experience,
        ...
        const formattedJobs = data.map((job) => ({
          ...
          applyLink:
            job.applyLink ||
            `mailto:careers@mactus.in?subject=Application for ${encodeURIComponent(
              job.title || ""
            )}`,
        ...
        console.error("Sanity jobs fetch error:", error);
        setStatus("error");
      }
    };
  ...
  const benefits = [
    {
      label: "Cutting edge digital compliance management platforms",
      icon: <FaRocket />,
    },
    {
      label: "Career growth and learning opportunities",
      icon: <FaChartLine />,
    },
    {
      label: "Collaborative and supporting environment",
      icon: <FaHandshake />,
    },
    {
      label: "Competitive remuneration and benefits",
            ...
            {status === "success" &&
              jobs.map((job) => (
                <JobCard
                  key={job.id}
                  job={job}
                  onApply={() => setIsResumeModalOpen(true)}
                />
              ))}
